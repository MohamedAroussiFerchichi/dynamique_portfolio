...
// Gestion de l'upload de photo (admin uniquement)
if ($is_admin && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['upload_photo'])) {
    if (!empty($_FILES['photo']['name'])) {
        $allowed  = ['jpg','jpeg','png','webp'];
        $ext      = strtolower(pathinfo($_FILES['photo']['name'], PATHINFO_EXTENSION));
        if (!in_array($ext, $allowed)) {
            $error = "Format de photo non autorisé. Utilisez JPG, PNG ou WEBP.";
        } else {
            $filename = 'photo_' . time() . '.' . $ext;
            $dest     = "fichiers/" . $filename;
            if (move_uploaded_file($_FILES['photo']['tmp_name'], $dest)) {
                if ($info) {
                    mysqli_query($conn, "UPDATE infos_personnelles SET photo='$filename' WHERE id=" . (int)$info['id']);
                } else {
                    mysqli_query($conn, "INSERT INTO infos_personnelles (photo) VALUES ('$filename')");
                }
    ...
    <div style="display:grid;grid-template-columns:1fr 2fr;gap:2.5rem;align-items:start">
        ...
        <div>
            <div class="info-block" style="text-align:center;margin-bottom:1.5rem">
                ...
                <?php else: ?>
                    <img src="fichiers/profile.png" alt="Photo par défaut" style="width:160px;height:160px;object-fit:cover;border-radius:50%;border:3px solid var(--accent);margin-bottom:1rem;display:block;margin-left:auto;margin-right:auto;">
                <?php endif; ?>
        ...
        <div>
            ...
            <div class="info-block" style="border-color:rgba(108,99,255,0.4)">
                <h3>Admin – Mettre à jour la photo</h3>
                <form method="POST" enctype="multipart/form-data" style="margin-bottom: 2rem;">
                    <div class="form-group">
                        <label>Fichier Photo (JPG, PNG, WEBP)</label>
                        <input type="file" name="photo" accept=".jpg,.jpeg,.png,.webp" required>
                    </div>
                    <button type="submit" name="upload_photo" class="btn btn-primary">Uploader la photo</button>
                </form>

                <h3>Admin – Mettre à jour le CV</h3>
                <form method="POST" enctype="multipart/form-data">
                    <div class="form-group">
                        <label>Fichier CV (PDF, DOC, DOCX)</label>
                        <input type="file" name="cv" accept=".pdf,.doc,.docx" required>
                    </div>
                    <button type="submit" name="upload_cv" class="btn btn-primary">Uploader le CV</button>
                </form>
            </div>
            <?php endif; ?>
        </div>
