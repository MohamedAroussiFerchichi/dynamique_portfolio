    ...
    <div style="display:flex;flex-direction:column;gap:1rem">
        ...
        <div class="cert-card animate-in">
            ...
            <div class="cert-actions">
                <?php if ($c['fichier']): ?>
                    <a href="fichiers/<?= htmlspecialchars($c['fichier']) ?>" target="_blank" class="btn btn-success btn-sm">Voir</a>
                <?php endif; ?>
                <?php if ($is_admin): ?>
                    <a href="modifier_certificat.php?id=<?= $c['id'] ?>" class="btn btn-secondary btn-sm">Modifier</a>
                    <a href="supprimer_certificat.php?id=<?= $c['id'] ?>" class="btn btn-danger btn-sm"
                       onclick="return confirm('Supprimer ce certificat ?')">Supprimer</a>
                <?php endif; ?>
            </div>
        </div>
        <?php endwhile; ?>
